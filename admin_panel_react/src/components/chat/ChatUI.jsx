...
function getParentOrigin() {
    ...
    // Fallback: si por alguna razón no hay referrer, usamos nuestro origin
...
export default function ChatUI({ embed = false, placeholder = "Escribe un mensaje…" }) {
    ...
    const isAllowed = useCallback(
        (origin) => normalize(origin) === parentOrigin || (allowed.length > 0 && allowed.includes(normalize(origin))),
    ...
    useEffect(() => {
        const onMsg = (ev) => {
            ...
            // (Opcional) settings/auth:
            // if (data.type === "chat:settings") { ... }
            // if (data.type === "auth:token") { setToken(data.token) }
        };
                                                    ...
                                                    {m.card.buttons.map((b) => (
                                                        <IconTooltip key={b.id} label={b.payload || b.url || "Acción"} side="top">
                                                            <button
                                                                ...
                                                                aria-label={b.title}
                                                            >
                                                                {b.url ? <ExternalLink className="w-3.5 h-3.5" aria-hidden="true" /> : null}
                                                                {b.title}
                                                            </button>
                                                        </IconTooltip>
                                                    ))}
