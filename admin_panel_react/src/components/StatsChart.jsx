...
import { Download, RefreshCw, BarChart2, Users as UsersIcon, FileText } from "lucide-react";
...
function StatsChart({ desde, hasta }) {
    ...
    const fetchData = async () => {
        try {
            ...
            // Normaliza exportaciones (acepta array o objeto {fecha: total})
            const normalizedExports = Array.isArray(exportsRes)
                ? exportsRes
                : exportsRes && typeof exportsRes === "object"
                    ? Object.entries(exportsRes).map(([k, v]) => ({ _id: k, total: v }))
                    : [];

            setStats(statsRes);
            setExportData(normalizedExports);
        } catch (err) {
    ...
    // Normaliza intents_mas_usados → { name, total }
    const intentsData = useMemo(() => {
        const list = stats?.intents_mas_usados ?? [];
        return list.map((i) => ({
            name: i.intent ?? i._id ?? "—",
    ...
    const handleExportCSV = () => {
        ...
        // Exporta exactamente lo que se grafica en “Exportaciones por Día”
        ...
        >
            ...
            <div className="col-span-2 flex justify-end gap-3 mb-2">
                <IconTooltip label="Recargar estadísticas" side="top">
                    <Button onClick={fetchData} className="inline-flex items-center gap-2" type="button">
                        <RefreshCw size={16} /> Recargar
                    </Button>
                </IconTooltip>
                <IconTooltip label="Exportar CSV (exportaciones por día)" side="top">
                    <Button onClick={handleExportCSV} className="inline-flex items-center gap-2" type="button">
            ...
            <motion.div whileHover={{ scale: 1.02 }}>
                ...
                <ul className="list-disc list-inside text-sm text-gray-700 dark:text-gray-300">
                    ...
                    <li>
                        <strong>Exportaciones CSV:</strong> {stats.total_exportaciones_csv}
                    </li>
                </ul>
            </motion.div>
            ...
            {/* Intents más usados */}
            <motion.div whileHover={{ scale: 1.02 }}>
                <h2 className="font-semibold text-lg mb-2 dark:text-white">Intents más usados</h2>
                        ...
                        <tbody>
                            {(stats.ultimos_usuarios ?? []).map((u, index) => (
                                <tr key={index} className="border-t dark:border-gray-700">
                                    <td className="p-2 text-gray-700 dark:text-gray-200">{u.email}</td>
                                    <td className="p-2">
                                        <Badge type="role" value={u.rol} />
                                    </td>
                                    <td className="p-2 text-xs text-gray-500 dark:text-gray-400">{u._id}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
