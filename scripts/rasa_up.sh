...
echo "🧠 Validando & entrenando (opcional si ya entrenaste)..."
( cd "${RASA_DIR}" && rasa data validate && rasa train ) || true

echo "⚙️  Iniciando Action Server en :${ACTIONS_PORT}..."
...
echo "🚀 Iniciando Rasa HTTP API en :${RASA_PORT} (CORS abierto para pruebas)..."
( cd "${RASA_DIR}" && rasa run --enable-api -p "${RASA_PORT}" --cors "*" ) \
  > "${STATE_DIR}/rasa.log" 2>&1 &
echo $! > "${RASA_PID_FILE}"

echo "✅ Listo:"
echo "• Actions   : http://localhost:${ACTIONS_PORT}/webhook"
echo "• Rasa API  : http://localhost:${RASA_PORT}"
echo "🪵 Logs: ${STATE_DIR}/actions.log y ${STATE_DIR}/rasa.log"